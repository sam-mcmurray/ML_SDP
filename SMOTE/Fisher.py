...
# Run the model using fisher score for feature selection with smote
def run_fisher(X, Y, i, model, kf) -> list:
    # create scorer
    ...
    pipe = Pipeline(steps=[('smote', SMOTE(random_state=42)),
                           ...
                           ('skb', SelectKBest(fisher_score.fisher_score, k=i)),
                           ('model', model)])
...
def find_n_fisher_features(max_features, x, y, model, kf):
    print("Fisher")
...
def start_fisher_experiment(model, file, path_to_directory, results_file, kf):
    ...
    best_features = find_n_fisher_features(max_features, x, y, model, kf)
    # Run the model and get the results
    scores = run_fisher(x, y, best_features, model, kf)
    # Save the results
